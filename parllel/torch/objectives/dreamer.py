...
class DreamerModelLoss(Loss):
    ...
    Args:
        ...
        lambda_reconstruction (float, optional): the weight of the reconstruction loss. Default: 1.0.
    ...
    def loss(self, samples: ArrayDict[Tensor]) -> tuple[Tensor, AlgoInfoType]:
        ...
        kl_loss = self.kl_loss(
            ...
            prior_std=samples["prior_std"],
            posterior_mean=samples["posterior_mean"],
            posterior_std=samples["posterior_std"],
        )
        reconstruction_loss = F.mse_loss(
            samples["pixels"],
            samples["reconstructed_pixels"],
            reduction="none",
        )
        if not self.global_average:
            reconstruction_loss = reconstruction_loss.sum((-3, -2, -1))
        reconstruction_loss = reconstruction_loss.mean()

        reward_loss = F.mse_loss(
            samples["reward"],
            samples["reconstructed_reward"],
            reduction="none",
        )
        if not self.global_average:
        ...
        info = {
            ...
            "loss_model_reco": self.lambda_reconstruction * reconstruction_loss,
            "loss_model_reward": self.lambda_reward * reward_loss,
        }

        return sum(info.values()), {key: value.item() for key, value in info.items()}
    ...
    def kl_loss(
        ...
        # CHECK: isn't this just KL between two Gaussians? why not use torch distributions?
        kl = (
